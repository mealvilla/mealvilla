...
const DestinationPage: React.FC = () => {
  ...
  const destination = destinations.find(d => d.id === name);

  useEffect(() => {
    if (destination) {
      document.title = `Properties in ${destination.name} - MealVilla`;
    }
  }, [destination]);
  ...
  const filteredProperties = useMemo(() => {
    let props = properties.filter(p => p.destination === name);
    ...
    return props;
  }, [name, ratingFilter, priceFilter, typeFilters, amenityFilters, sortOrder]);
  ...
  return (
    <div className="bg-bg-light py-12 md:py-20">
      <div className="container mx-auto px-6">
        <h1 className="text-4xl md:text-5xl font-bold font-serif text-accent mb-4">Explore {destination.name}</h1>
        <p className="text-lg text-text-muted max-w-3xl mb-12">{destination.description}</p>
