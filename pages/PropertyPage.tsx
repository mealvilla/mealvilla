...
const PropertyPage: React.FC = () => {
  ...
  const whatsappLink = `https://wa.me/${property.contactPhone}?text=Hi! I'm interested in booking ${property.name}.`;
          ...
          <div className="lg:col-span-3">
            ...
            <div className="mt-8">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-4">
                <h1 className="text-3xl md:text-4xl font-bold font-serif text-black mb-2 sm:mb-0">{property.name}</h1>
                <div className="flex items-center">
                  <StarRating rating={property.rating} />
                  <span className="text-lg text-text-muted ml-3">{property.rating.toFixed(1)}</span>
                </div>
              </div>
              <p className="text-lg text-text-muted mb-6">{property.location}</p>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white p-8 rounded-lg shadow-xl sticky top-28">
              <p className="text-3xl font-bold text-accent mb-4">
                ₹{property.pricePerNight.toLocaleString('en-IN')}<span className="text-base font-normal text-text-muted">/night</span>
              </p>
              <a 
                href={whatsappLink} 
                target="_blank" 
